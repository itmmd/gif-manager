...
>
    ...
    <div class="relative max-w-4xl mx-auto" style="z-index:1;">
        <div
            ...
            style="background: linear-gradient(135deg, rgba(99,102,241,0.18) 0%, rgba(139,92,246,0.14) 50%, rgba(6,182,212,0.10) 100%); border: 1px solid rgba(99,102,241,0.25);"
        >
            ...
            <div aria-hidden="true" class="pointer-events-none absolute inset-0 overflow-hidden">
                <div style="position:absolute; top:-30%; left:-10%; width:60%; height:80%; background:radial-gradient(circle, rgba(99,102,241,0.20) 0%, transparent 65%); filter:blur(60px);"></div>
                <div style="position:absolute; bottom:-20%; right:-5%; width:50%; height:70%; background:radial-gradient(circle, rgba(6,182,212,0.15) 0%, transparent 65%); filter:blur(60px);"></div>
            </div>

            <div class="relative" style="z-index:2;">

                <div class="inline-flex items-center gap-2 rounded-full text-xs font-semibold tracking-widest uppercase mb-6"
                     style="background:rgba(99,102,241,0.15); border:1px solid rgba(99,102,241,0.30); padding:0.35rem 1rem; color:#a5b4fc;">
                    {{ $cta['eyebrow'] }}
                </div>
                ...
                <h2 class="font-black tracking-tight text-white mb-5"
                    ...
                    {{ $cta['headline'] }}<br>
                    <span style="background: linear-gradient(130deg,#a5b4fc,#818cf8,#c084fc,#67e8f9); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;">{{ $cta['headline_em'] }}</span>
                </h2>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                    <div
                        x-data="{
                            ...
                            addRipple(e) {
                                ...
                                setTimeout(() => { this.ripples = this.ripples.filter(r => r.id !== id); }, 700);
                            }
                        }"
                    ...
                    >
                        <a
                            ...
                            style="background: linear-gradient(135deg,#6366f1,#8b5cf6,#06b6d4); padding: 1rem 2.4rem; font-size: 1.05rem; box-shadow: 0 0 40px rgba(99,102,241,0.55), 0 4px 24px rgba(99,102,241,0.35); transition: box-shadow 0.3s ease, transform 0.2s ease;"
                        ...
                        <template x-for="r in ripples" :key="r.id">
                            <span class="ripple-circle pointer-events-none absolute rounded-full" :style="`left:${r.x}px; top:${r.y}px;`" aria-hidden="true"></span>
                        </template>
                    </div>

                    <a href="{{ route('login') }}" class="text-sm text-slate-400 hover:text-slate-200 transition-colors">
