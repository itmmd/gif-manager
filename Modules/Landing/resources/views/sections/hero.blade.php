...
>
    ...
    <div class="relative z-10 mx-auto flex max-w-4xl flex-col items-center px-6 pt-28 pb-20 text-center sm:pt-32">
        ...
        <div data-reveal class="glass mb-8 inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-medium text-slate-300">
        ...
        <h1 data-reveal data-reveal-delay="1" class="type-display max-w-3xl text-3xl font-extrabold text-white sm:text-5xl lg:text-5xl">
            {{ $hero['headline'] }}
            <span class="text-gradient glow-text-primary">{{ $hero['headline_em'] }}</span>
            {{ $hero['headline_tail'] }}
        </h1>

        <p data-reveal data-reveal-delay="2" class="type-body mt-6 max-w-2xl text-lg text-slate-400 sm:text-xl">
            {{ $hero['subtitle'] }}
        </p>

        <div data-reveal data-reveal-delay="3" class="mt-10 flex w-full flex-col items-center justify-center gap-4 sm:w-auto sm:flex-row">
            <a
                href="{{ route('register') }}"
        ...
        <div data-reveal data-reveal-delay="4" class="mt-14 flex flex-col items-center gap-3 sm:flex-row sm:gap-6">
            ...
            <div class="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-sm font-semibold text-slate-600">
                @foreach ($hero['trust_logos'] as $logo)
                    <span>{{ $logo }}</span>
                    @if (!$loop->last)
    ...
    <div data-reveal data-reveal-delay="5" class="absolute bottom-8 left-1/2 -translate-x-1/2" aria-hidden="true">
