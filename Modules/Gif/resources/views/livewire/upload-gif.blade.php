                        ...
                        <div class="alert alert-warning" role="alert" aria-live="assertive" style="margin-bottom:20px;">
                            ...
                            <div class="alert-body">
                                ...
                                <div style="margin-top:10px; display:flex; gap:8px;">
                                    <button wire:click="confirmDuplicate" class="btn btn-warning" style="height:32px; padding:0 14px; font-size:13px;" aria-label="Upload anyway">Upload anyway</button>
                                    <button wire:click="$set('file', null)" class="btn btn-outline" style="height:32px; padding:0 14px; font-size:13px;" aria-label="Choose different file">Choose different file</button>
                                </div>
                            </div>
                        </div>
                    @endif
                        ...
                        @if ($file && !$errors->has('file'))
                            <div class="form-group" aria-live="polite">
                                ...
                                <div style="display:inline-block; border-radius:var(--radius); overflow:hidden; border:1px solid var(--border-color); background:var(--card-bg);">
                                    @if ($ext === 'mp4')
                                        <video src="{{ $file->temporaryUrl() }}" controls muted playsinline style="max-width:320px; max-height:240px; display:block;" aria-label="MP4 preview"></video>
                                    @else
                                        <img src="{{ $file->temporaryUrl() }}" alt="GIF preview" style="max-width:320px; max-height:240px; display:block;">
                                    @endif
                                </div>
                                <p class="form-hint" style="margin-top:6px;">{{ $origName }} — {{ round($file->getSize() / 1024, 1) }} KB</p>
                            </div>
                        @endif

                        <div class="form-actions right" style="border-top:none; margin-top:8px; padding-top:0;">
                            <button type="submit" class="btn btn-brand" wire:loading.attr="disabled" wire:target="save">
...
.dropzone__icon { color: var(--text-muted, #94a3b8); margin-bottom: 10px; }
.dropzone__label { font-size: .875rem; color: var(--text-secondary, #cbd5e1); margin: 0 0 4px; }
.dropzone__hint { font-size: .75rem; color: var(--text-muted, #64748b); margin: 0; }
.dropzone__loading { text-align: center; }
...
.btn-brand:hover { box-shadow: 0 6px 20px rgba(99,102,241,.50); transform: translateY(-1px); }
