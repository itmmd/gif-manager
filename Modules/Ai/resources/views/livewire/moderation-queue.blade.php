<div>
    ...
    <div style="display:flex; gap:8px; margin-bottom:20px;">
        <button wire:click="setFilter('flagged')" class="btn {{ $filter === 'flagged' ? 'btn-danger' : 'btn-outline' }}"
        ...
        <button wire:click="setFilter('pending_review')" class="btn {{ $filter === 'pending_review' ? 'btn-warning' : 'btn-outline' }}"
                style="height:32px; padding:0 14px; font-size:13px; display:flex; align-items:center; gap:6px;"
        ...
        <button wire:click="setFilter('all')" class="btn {{ $filter === 'all' ? 'btn-primary' : 'btn-outline' }}"
                style="height:32px; padding:0 14px; font-size:13px;"
                aria-pressed="{{ $filter === 'all' ? 'true' : 'false' }}">All</button>
    </div>
                ...
                <div class="media-grid">
                    @foreach ($gifs as $gif)
                        <div class="media-tile" wire:key="mod-{{ $gif->id }}" style="position:relative;">
                            ...
                            @else
                                <img src="{{ $gif->url }}" alt="{{ e($gif->title) }}" loading="lazy"
                                     style="width:100%;height:100%;object-fit:cover;display:block;">
                            @endif

                            @if ($gif->isFlagged())
                                <div style="position:absolute;top:6px;left:6px;background:rgba(239,68,68,0.9);color:#fff;border-radius:4px;font-size:10px;font-weight:600;padding:2px 6px;">Flagged</div>
                            @elseif ($gif->isPendingReview())
                                <div style="position:absolute;top:6px;left:6px;background:rgba(251,191,36,0.9);color:#000;border-radius:4px;font-size:10px;font-weight:600;padding:2px 6px;">Pending</div>
                            @endif
                            ...
                            @if ($confirmRejectId === $gif->id)
                                <div style="position:absolute;inset:0;background:rgba(15,23,42,0.9);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px;"
                                    ...
                                    <div style="display:flex;gap:6px;">
                                        <button wire:click="reject({{ $gif->id }})" class="btn btn-danger"
                                                style="height:28px;padding:0 10px;font-size:12px;" aria-label="Confirm rejection">Delete</button>
                                        <button wire:click="cancelReject" class="btn btn-outline"
                                                style="height:28px;padding:0 10px;font-size:12px;" aria-label="Cancel">Cancel</button>
                                    </div>
                                </div>
                            @else
                                <div style="position:absolute;top:6px;right:6px;display:flex;gap:4px;">
                                    <button wire:click="approve({{ $gif->id }})"
                                            ...
                                            title="Approve — make public" aria-label="Approve {{ e($gif->title) }}">
                                    ...
                                    <button wire:click="confirmReject({{ $gif->id }})"
                                            ...
                                            title="Reject and delete" aria-label="Reject {{ e($gif->title) }}">
